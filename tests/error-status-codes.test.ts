...
describe("NWP_ERROR_TO_NPS_STATUS mapping — 53 codes", () => {
  ...
  it("HTTP binding rejection codes map to their canonical NPS statuses", () => {
    expect(NWP_ERROR_TO_NPS_STATUS["NWP-HTTP-ORIGIN-FORBIDDEN"]).toBe("NPS-AUTH-FORBIDDEN");
    expect(NWP_ERROR_TO_NPS_STATUS["NWP-HTTP-CONTENT-TYPE-UNSUPPORTED"]).toBe("NPS-CLIENT-BAD-FRAME");
    expect(NWP_ERROR_TO_NPS_STATUS["NWP-HTTP-ACCEPT-UNSATISFIABLE"]).toBe("NPS-CLIENT-BAD-PARAM");
    expect(NWP_ERROR_TO_NPS_STATUS["NWP-HTTP-REQUEST-ID-MISMATCH"]).toBe("NPS-CLIENT-BAD-PARAM");
    expect(NWP_ERROR_TO_NPS_STATUS["NWP-HTTP-FRAME-BODY-MALFORMED"]).toBe("NPS-CLIENT-BAD-FRAME");
    expect(NWP_ERROR_TO_NPS_STATUS["NWP-CAPABILITY-ADVERTISED-UNIMPLEMENTED"]).toBe("NPS-SERVER-UNSUPPORTED");
  });
